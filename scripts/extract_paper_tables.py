        ...
        if '\\begin{table}' in lines[i]:
            ...
            while i < len(lines) and '\\end{table}' not in lines[i]:
                if '\\caption{' in lines[i]:
                    ...
                    elif '\\catcoding' in caption_text or 'code' in caption_lower or 'coding' in caption_lower:
                        table_data['category'] = 'code'
                    elif '\\catdata' in caption_text or 'data analysis' in caption_lower:
                        table_data['category'] = 'data'
                    elif '\\catendtoend' in caption_text or 'end-to-end' in caption_lower or 'discovery' in caption_lower:
                        table_data['category'] = 'discovery'
                    
                    # Also check for specific dataset names
                    if 'paperfindingbench' in caption_lower or 'paper finding' in caption_lower:
                        table_data['category'] = 'lit_search'
                    elif 'scholarqa' in caption_lower or 'litqa' in caption_lower:
                        table_data['category'] = 'lit_qa'
                    elif 'arxivdigestables' in caption_lower or 'table' in caption_lower and 'literature' in caption_lower:
                ...
                if '&' in lines[i] and '\\\\' in lines[i]:
                    ...
                    if not any(rule in line for rule in ['\\toprule', '\\midrule', '\\bottomrule', '\\cmidrule', '\\addlinespace']):
                        # Check if it's a header row (contains 'Score', 'Cost', 'Agent', 'Model', etc.)
                        if not any(header in line for header in ['Score', 'Cost', 'Agent', 'Model', ' O ', ' T ']):
...
def identify_table_type(table: Dict) -> Optional[str]:
    ...
    if 'paperfindingbench' in caption or 'paper finding' in caption or 'literature search' in caption:
        return 'lit_search'
    elif 'scholarqa' in caption or 'literature qa' in caption:
        return 'lit_qa'
    elif 'arxivdigestables' in caption or ('table' in caption and 'synthesis' in caption):
        return 'lit_table'
    elif 'super' in caption or 'core-bench' in caption or 'ds-1000' in caption or 'coding' in caption:
        return 'code'
...
def main():
    ...
    parser.add_argument(
        "tex_file",
        type=str,
        help="Path to the LaTeX file to extract tables from"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Show detailed output including all rows"
    )
    ...
    for i, table in enumerate(tables, 1):
        ...
        print(f"  Caption: {table['caption'][:100]}{'...' if len(table['caption']) > 100 else ''}")
