...
def calculate_model_usages_cost(model_usages: list[list[ModelUsageWithName]], model_costs: Dict[str, Dict[str, float]]) -> float:
    total_cost = 0.0
        ...
        for usage_with_name in problem_usages:
            ...
            if model_name in model_costs:
                ...
                if usage.input_tokens_cache_write is not None:
                    usage_cost += usage.input_tokens_cache_write * costs.get('input_tokens_cache_write', 0.0)
                
                if usage.input_tokens_cache_read is not None:
                    usage_cost += usage.input_tokens_cache_read * costs.get('input_tokens_cache_read', 0.0)
                
                if usage.reasoning_tokens is not None:
                    usage_cost += usage.reasoning_tokens * costs.get('reasoning_tokens', 0.0)
...
def test_compression_preserves_total_cost(sample_model_usages, sample_model_costs):
    ...
    compressed_cost = calculate_model_usages_cost(compressed_model_usages, sample_model_costs)
    
    # Costs should be identical
    assert abs(original_cost - compressed_cost) < 1e-10, f"Original cost: {original_cost}, Compressed cost: {compressed_cost}"
